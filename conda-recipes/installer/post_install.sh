...
do_chemps2 = True
do_pcmsolver = True
do_v2rdm = True if _platform in ['linux', 'linux2'] else False
do_ambit = True if _platform == 'darwin' else False
...
PSIOUT=`unset PSIDATADIR; PSI_SCRATCH=/tmp; PATH=$PREFIX/bin:$PATH; psi4 -i linktest.in -o linktest.out >> .messages.txt`
...
PSISOOUT=`unset PSIDATADIR; PSI_SCRATCH=/tmp; PATH=$PREFIX/bin:$PATH; python linktest.py >> .messages.txt`
