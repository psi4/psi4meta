...
echo "    Inputs:  ${SP_DIR}/v2rdm_casscf/tests" >> .messages.txt
...
if [ "${CONDA_PY}" == "27" ]; then
    ...
    echo "      psi4 \"\$(dirname \$(which psi4))\"/../lib/python3.5/site-packages/v2rdm_casscf/tests/v2rdm1/input.dat -o output.dat" >> .messages.txt
fi
echo "" >> .messages.txt
