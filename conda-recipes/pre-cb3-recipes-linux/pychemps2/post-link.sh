set +x

echo "" >> .messages.txt
echo "  Thank you for installing PyCheMPS2." >> .messages.txt
echo "    Manual:  sebwouters.github.io/CheMPS2" >> .messages.txt
echo "    GitHub:  github.com/SebWouters/CheMPS2" >> .messages.txt
echo "    Binary:  anaconda.org/psi4/pychemps2" >> .messages.txt
#echo "    Inputs:  ${PREFIX}/share/psi4/samples/chemps2" >> .messages.txt
#echo "    Test (after first activating conda installation or environment):" >> .messages.txt
#echo "      python \"\$(dirname \$(which python ))\"/../lib/python2.7/site-packages/ambit/tests/test_operators.py" >> .messages.txt
echo "" >> .messages.txt

